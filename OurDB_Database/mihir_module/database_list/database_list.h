...
std::string databaseList()
{
    std::filesystem::path path(strPath[0]);   //path where all our databases are created and stored
    std::string list;   //string to store the list of databases

    if(!std::filesystem::is_empty(path))     //checks if the current path to database is empty or not
    {
        list += "Databases are : \n";
        for (auto &i : std::filesystem::directory_iterator(path))    //increments the existing folders(databases) at given path
        {
            if(i.is_directory())
            {
